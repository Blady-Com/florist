...
procedure p140100 is
   ...
   begin
      ...
      Assert (Get_Initial (State) = Zero_Timespec, "A003");
      Assert (Get_Interval (State) = Zero_Timespec, "A004");
      ...
      Assert (Get_Initial (State) = Zero_Timespec,
        "A005: Get_Initial after disarm /= 0");
      Assert (Get_Interval (State) <= To_Timespec (1, 0),
        "A006: Get_Interval after disarm > initial value");
      ...
      begin
         ...
         Set_Interval (State, Zero_Timespec);
         Arm_Timer (Timer, Absolute_Timer, State);
         Assert (False, "A007: zero initial value not detected");
         Disarm_Timer (Timer);
      exception
      when E1 : POSIX_Error =>
         if Get_Error_Code /= Invalid_Argument then
            Optional (Timers_Option, Operation_Not_Implemented, E1, "A008");
         end if;
      ...
      if Mode = Relative then
         ...
         begin
            ...
            Set_Interval (State, Zero_Timespec);
            Arm_Timer (Timer, Absolute_Timer, State);
            Assert (False, "A010: negative initial value not detected");
            Disarm_Timer (Timer);
         exception
         when E1 : POSIX_Error =>
            if Get_Error_Code /= Invalid_Argument then
               Optional (Timers_Option, Operation_Not_Implemented, E1, "A011");
            end if;
      ...
      --  causes it to go off immediately.
         ...
         begin
            ...
            State := Get_Timer_State (Timer);
            Assert (Get_Initial (State) = Zero_Timespec,
              "A013: Get_Initial after expiration /= 0");
            Assert (Get_Interval (State) = Zero_Timespec,
              "A014: Get_Interval after expiration /= 0");
            Assert (Get_Timer_Overruns (Timer) = 0,
              "A015: nonzero overruns");
         exception
         when E1 : POSIX_Error =>
            Optional (Timers_Option, Operation_Not_Implemented, E1, "A016");
         ...
         while PDelta < To_Timespec (2, 0) loop
            ...
            if Temp > Zero_Timespec then
               ...
               if Get_Seconds (Temp) > 1 then
                  Assert (Interval_Roundup,
                    "A018: unbelievable interval roundup: "
                    & Image (Temp));
            ...
            while not Failed loop
               ...
               if NowInitial >= LastInitial then
                  Assert (False, "A019: timer value nondecreasing");
            ...
            if not Failed then
               ...
               if Temp < PDelta then
                  Assert (False, "A020: early timer expiration: "
                    & Image (PDelta - Temp));
...
begin
   ...
   begin
      Assert (To_Timespec (0, 1) > Zero_Timespec, "A025");
      Assert (To_Timespec (-1, 0) < Zero_Timespec, "A026");
      Assert (To_Timespec (0, 0) = Zero_Timespec, "A027");
      Assert (To_Timespec (0, 1) + To_Timespec (0, 1)
        = To_Timespec (0, 2), "A028");
      Assert (To_Timespec (0, Nanoseconds'Last) + To_Timespec (0, 1)
        = To_Timespec (1, 0), "A029");
      Assert (To_Timespec (0, Nanoseconds'Last) - To_Timespec (0, 1)
        = To_Timespec (0, Nanoseconds'Last - 1), "A030");
      while PDelta < To_Timespec (2, 0) loop
         if PDelta < One_Second then
            Assert (PDelta = To_Timespec (0, N), "A031");
            Assert (N = Get_Nanoseconds (PDelta), "A032");
         end if;
         Assert (Get_Seconds (PDelta) < 2, "A033");
         Assert (PDelta > Zero_Timespec, "A034");
         PDelta := PDelta * 2;  N := N * 2;
      end loop;
      Assert (To_Timespec (987_654_321, 87_654_321) +
        To_Timespec (100_000_000, 10_000_000) =
        To_Timespec (1_087_654_321, 97_654_321), "A035");
      Assert (To_Timespec (987_654_321, 87_654_321) -
        To_Timespec (100_000_000, 10_000_000) =
        To_Timespec (887_654_321, 77_654_321), "A036");
      Assert (To_Timespec (0, 955_899) > Zero_Timespec, "A037");
      Assert (To_Timespec (0, 955_899) -
        To_Timespec (0, 955_900) = To_Timespec (-1, 1E9 - 1), "A038");
   exception
   when E : others => Unexpected_Exception (E, "A039");
   end;
   ...
   exception
   when E1 : POSIX_Error =>
      Optional (Timers_Option, Operation_Not_Implemented, E1, "A042");
   ...
   begin
      ...
      Assert (S = 0
        and NS <= POSIX_Limits.Portable_Clock_Resolution_Minimum, "A044");
   ...
   begin
      ...
      Clock_Realtime_Supported := True;
      Assert (Get_Seconds (Time) /= -1, "A047");
   ...
   begin
      ...
      loop
         ...
         if Adif > 2.0 then
            Fail ("A050: Clock_Realtime does not move");
      ...
      if Clock_Realtime_Moves then
         ...
         Split (Pmin, S, NS);
         Assert (S = 0 and
           NS <= POSIX_Limits.Portable_Clock_Resolution_Minimum, "A051");
   ...
   exception
   when E1 : POSIX_Error =>
      Optional (Timers_Option, Operation_Not_Implemented, E1, "A052");
   when E2 : others => Unexpected_Exception (E2, "A053");
   end;
   ...
   begin
      ...
      when E1 : POSIX_Error =>
         if Get_Error_Code /= Invalid_Argument then
            Optional (Timers_Option, Operation_Not_Implemented, E1, "A054");
         end if;
      ...
      when E1 : POSIX_Error =>
         if Get_Error_Code /= Invalid_Argument then
            Optional (Timers_Option, Operation_Not_Implemented, E1, "A056");
         end if;
      when E2 : others => Unexpected_Exception (E2, "A057");
      end;
   end;
   ...
   exception
   when E1 : POSIX_Error =>
      Optional (Timers_Option, Operation_Not_Implemented, E1, "A058");
   when E2 : others => Unexpected_Exception (E2, "A059");
   end;
...
when E1 : POSIX_Error =>
   ...
   Done;
when E2 : others => Fatal_Exception (E2, "A077");
end p140100;
