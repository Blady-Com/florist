...
/* Define a unique value even non supported to avoid case duplication choices.
*/
...
#define GT2(COMPNAME, CMPTYP)                 \
  ...
  #CMPTYP,                                    \
  sizeof (CMPTYP),                            \
  ((char *)&DUMMY.COMPNAME - (char *)&DUMMY), \
  0,
...
#define gen_unchckd_conv(FUNC, FROM, TO)                              \
  ifprintf (fp,                                                       \
            "   function %s is new Unchecked_Conversion (%s, %s);\n", \
            FUNC,                                                     \
            FROM,                                                     \
            TO);
...
void warn (const char   msg1[],  
           const char   msg2[]);
void quit (const char   msg1[],  
           const char   msg2[]);

void save_type (char const     name[],
                int            typesize,
                int            typekind,
                component_t*   comps);

void print_type_declaration (char const   name[],
                             FILE*        fp);
...
void gbrg     (char   name[],  
               char   lb[],
               char   ub[]);
void gsitp    (char   name[],
               int    size);
void gdflsitp (char   name[]);
void guitp    (char   name[],
               int    size);
...
void gptrtp   (char const   ptrname[], 
               char const   desname[]);

void gfunc    (char const   name[],
               char const   xname[],
               int          have_it);
void gfuncd   (char const   name[],
               char const   xname[],
               int          have_it);

void gmaxi    (char const   name[],
               int          lower_bound);
void gmaxii   (char const   name[], 
               int          lower_bound);
void gmaxn    (char const   name[], 
               int          lower_bound);
void gmaxnn   (char const   name[],
               int          lower_bound);

void gpmaxn   (char const   name[], 
               int          value);
void gpmaxr   (char const   name[], 
               char const   oname[]);
void grename  (char const   name[], 
               char const   oname[]);
...
void g_ssize_t() {gsitp ("ssize_t", sizeof (ssize_t));}
  ...
  void g_pthread_condattr_t() {gptp ("pthread_condattr_t", sizeof (pthread_condattr_t));}
#else
  ...
  void g_pthread_mutexattr_t() {gptp ("pthread_mutexattr_t", sizeof (pthread_mutexattr_t));}
#else
...
{
   gsitp ("suseconds_t", 
          sizeof (struct_timeval_dummy.tv_usec));
}
...
/* print_type_declaration
   ----------------------
   print out Ada type declaration for C type with specified name
*/

void
print_type_declaration (char const   name[],
                        FILE*        fp)
{
  type_t*               type;
  component_t const*    p;
  ...
  switch (type->typekind)
  {
    case SIGNED_INTEGER_TYPE:
      ifprintf (fp,
                "   type %s is range -2**%d .. (2**%d) - 1;\n",
                type->typename,
                type->typesize*bits_per_byte - 1,
                type->typesize*bits_per_byte - 1);

      ifprintf (fp,
                "   for %s'Size use %d;  --  Added\n",
                extended_name,
...
void
print_ada_type (char const   typename[])
{
  ...
  else if (!strcmp (typename, "struct in_addr"))         fprintf (fp, "struct_in_addr");
  else if (!strcmp (typename, "struct sigevent"))        fprintf (fp, "struct_sigevent");
  else if (!strcmp (typename, "struct sockaddr *"))      fprintf (fp, "sockaddr_ptr");
  else if (!strcmp (typename, "struct iovec *"))         fprintf (fp, "iovec_ptr");
  else if (!strcmp (typename, "struct addrinfo *"))      fprintf (fp, "addrinfo_ptr");
  else if (!strcmp (typename, "struct timespec"))        fprintf (fp, "struct_timespec");
  else if (!strcmp (typename, "struct netbuf"))          fprintf (fp, "struct_netbuf");
  else if (!strcmp (typename, "union sigval"))           fprintf (fp, "sigval");
  else if (!strcmp (typename, "pthread_attr_t *"))       fprintf (fp, "System.Address");
  else if (!strcmp (typename, "void (*)()"))             fprintf (fp, "System.Address");
  else if (!strcmp (typename, "volatile void *"))        fprintf (fp, "System.Address");
  else if (!strcmp (typename, "fd_mask"))                fprintf (fp, "fd_mask_array (1 .. %d)", 
                                                                      sizeof (fd_set) / sizeof (unsigned int));
  else if (!strncmp (typename, "void (*)(", 9)
           && typename [strlen (typename) - 1] == ')')   fprintf (fp, "System.Address");   /* translate "void (*)(...)" into System.Address */
...
wordsize (int   n)
{
  return (n % sizeof (unsigned int)) == 0 ?   n / sizeof (int):
                                              n / sizeof (int) + 1;
}
       ...
       int have_it)
{
  if (have_it)
  {
    ...
    {
      ifprintf (fp, "   %s_LINKNAME : constant String := \"%s\";\n", name, n2ln (name));
    }
  }
  else
  {
    ...
    if (strcmp (name, xname))
    {
      ...
      ifprintf (fp, "   --  expands to the real function name. This is risky ...\n");
...
#endif
    {
      ...
      ifprintf (fp, "   HAVE_%s : constant Boolean := False;\n",                name);
      ifprintf (fp, "   %s_LINKNAME : constant String := \"nosys_neg_one\";\n", name);
    }
  }
}
         ...
         int          have_it)
{
  ...
  else
  {
...
#endif
    {
      ...
      warn ("missing function ",                                                 name);
      ifprintf (fp, "   HAVE_%s : constant Boolean := False;\n",                 name);
      ifprintf (fp, "   %s_LINKNAME : constant String := \"notsup_neg_one\";\n", name);
    }
  }
}
        ...
        int          have_it)
{
  ...
  else 
  {
...
#endif
    {
      ...
      {
        ifprintf (fp, "   %s_LINKNAME : constant String := \"nosys_direct\";\n", name);
      }
    }
  }
}
          ...
          int          have_it)
{
  ...
  else
  {
...
#endif
    {
      ...
      {
        ifprintf (fp,"   %s_LINKNAME : constant String := \"notsup_direct\";\n", name);
      }
    }
  }
}
...
void
gmacrofunc (char const   funcname[],
            char const   parmtype[],
            char const   parmname[]) 
{
...
/* Run-Time Increasable Values
   These must be defined, but the actual limits, 
   as reported by sysconf(), may be higher.
*/
...
*/
  ...
  ifprintf (fp, "   function Is_Portable_Filename (Str : POSIX_String) return Boolean;\n");
  ifprintf (fp, "   function Is_Portable_Pathname (Str : POSIX_String) return Boolean;\n");
  ...
  ifprintf (fp, "   function Length (List : POSIX_String_List) return Natural;\n");
  ...
  ifprintf (fp, "   function Is_POSIX_Error  (Error : Error_Code) return Boolean;\n");
  ifprintf (fp, "   function Image           (Error : Error_Code) return String;\n");
  ifprintf (fp, "   No_Error : constant Error_Code := 0;\n");
  ifprintf (fp, "   --  Error code constants with negative values correspond to\n");
  ifprintf (fp, "   --  error codes that are not supported by the current system.\n\n");
  ...
  ifprintf (fp, "      range %d .. %d;\n\n",
                -1,
                XTI_ERROR_LAST-XTI_ERROR_FIRST + XTI_Error_First);
  ...
  ifprintf (fp, "      range %d .. %d;\n\n",
                -1,
                EAI_ERROR_LAST-EAI_ERROR_FIRST + EAI_Error_First);
  ...
  ifprintf (fp, "   subtype Nanoseconds   is Nanoseconds_Base range 0 .. (10**9) - 1;\n");
  ...
  ifprintf (fp, "   function Get_Nanoseconds (Time : Timespec) return Nanoseconds;\n");
    ...
    if (sizeof (unsigned short) == 2)
    {
      ...
      */
      ...
      if (x.l == 0x00010203)
      {
         ...
         ifprintf (fp, "   Host_Byte_Order_Is_Net_Byte_Order : Boolean := True;  --  Added\n\n");
      } 
      else
      {
         network_byte_order = 0;
         ifprintf (fp, "   Host_Byte_Order_Is_Net_Byte_Order : Boolean := False;  --  Added\n\n");
      }
    } 
    else
    {
      quit ("short is not 16-bit", "");
    }
  }
  else
  {
    quit ("int is not 32-bit", "");
  }
  
  ifprintf (fp, "   function Host_To_Network_Byte_Order (Host_32 : Interfaces.Unsigned_32)\n");
  ifprintf (fp, "      return Interfaces.Unsigned_32;\n");

  ifprintf (fp, "   function Host_To_Network_Byte_Order (Host_16 : Interfaces.Unsigned_16)\n");
  ifprintf (fp, "      return Interfaces.Unsigned_16;\n");

  ifprintf (fp, "   function Network_To_Host_Byte_Order (Host_32 : Interfaces.Unsigned_32)\n");
  ifprintf (fp, "      return Interfaces.Unsigned_32;\n");

  ifprintf (fp, "   function Network_To_Host_Byte_Order (Host_16 : Interfaces.Unsigned_16)\n");
  ifprintf (fp, "      return Interfaces.Unsigned_16;\n");
  ...
  ifprintf (fp, "   --  efficient arithmetic on that type than on a two-part C struct.\n");
  ifprintf (fp, "   --  We rely that GNAT implements type Duration with enough\n");
  ifprintf (fp, "   --  precision (64 bits) to hold a full C timespec value.\n");
  ifprintf (fp, "   --  The enclosing record is to permit implicit initialization.\n");
  ...
  { 
    ...
    for (i = 1;  i < 32;  i++)
    {
      ifprintf (fp, "   Option_%d  : constant Option_Set := (Option => 2**%d);\n",  i,  i - 1);
    }
  }
...
/* Try to find out the range of valid signals.
   ...
   limit.
   We need this because we want to represent sets of signals as
   Boolean arrays.
...
/* ......need to figure what to do with functions, next..... */
  ...
  */
  ...
  GFUNCD (pthread_mutexattr_getprioceiling, HAVE_pthread_mutexattr_getprioceiling);
  ...
  GFUNCD (pthread_mutexattr_setprioceiling, HAVE_pthread_mutexattr_setprioceiling);
  GFUNCD (pthread_mutexattr_setprotocol,    HAVE_pthread_mutexattr_setprotocol);
  /* if not supported, the error code is Operation_Not_Implemented, i.e., ENOSYS */
...
*/
  ...
  ifprintf (fp, "   type fd_mask_array is array (Integer range <>) of unsigned_int;\n");
